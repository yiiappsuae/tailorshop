...
class ProductModifierSetController extends Controller
{
    ...
    public function add_selected_modifiers(Request $request)
    {
        ...
        $metrial_id = $request->input('metrial_id');
        $farah_id = $request->input('farah_id');
        $metrial_need = $request->input('metrial_need');

        $quantity = $request->input('quantity', 1);
        $measure = $request->input('measure');
        $modifiers = Variation::whereIn('id', $selected)
                        ->get();
        $measures = explode(',', $measure);  
        $measurements='';
         foreach($measure as $mm){
           // array_push($measurements,$mm);
            $measurements .=$mm.',';
        ...
        {
            $modifier->measure=$measurements[$i];
            $i=$i+1;
        }           

        $metrial = Product::where('id', $metrial_id)
                        ->with(['name','unit'])
                        ->first(); 
        $farah = Product::
        leftJoin('variations', 'variations.product_id', '=', 'products.id')
        ->where('variations.id', $farah_id)
        ->with(['products.name','products.unit'])
        ->first();
        
        if (count($modifiers) > 0) {
        //    return $modifiers;
          return view('restaurant.product_modifier_set.add_selected_modifiers')
             ->with(compact('modifiers', 'index','farah', 'quantity', 'measure','selected','i','metrial_id','metrial_need','metrial','farah_id'));
        } else {
            return '';
        }
    }
