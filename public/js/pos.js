$(document).ready(function() {
    ...
    $('select#select_location_id').on('change', function() {
        reset_pos_form();
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var line_total = entered_qty * unit_price_inc_tax;
		 ...
		 var unit_cost_price = __read_number(tr.find('input.single_purchase_price'));
		   var total_cost = entered_qty * unit_cost_price;
		   tr.find('input.pos_line_purchase_total').val(total_cost);
    ...
    //It doesnt make sense to have select#select_location_id.on('change') here  
    // when there is a another event above with $('select#select_location_id').on('change', function() line 26
    ...
    $('select#product_category, select#product_brand').on('change', function(e) {
        ...
        //#Put the category name in the input.
        //$("#product_category option:selected").text();
        $('input#name_category').val($("#product_category option:selected").text());
/*         my_button = $('select#product_category').val();
        $('button#bt3').trigger('focus');
        $('#product_category_pos name:').trigger('focus'); */
        //console.log('LocationValue: ' + $("#product_category option:selected").val() + ' Nombre: ' + $("#product_category option:selected").text() );
        get_featured_products();
    });

    //#JCN Begin: Added to show categories list in the window above products (div)
    //I use the function .change() or .trigger('change') to trigger the event change in select#product_category
    $('button#product_category').on('click', function(e) {
        ...
        $('select#product_category').trigger('change'); //Trigger the event above     
    });
    // #JCN End
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (item_addtn_method == 0) {
        ...
        var is_added = false;
        //alert('#JCNAlert Entrando a colocar productos item_addtn_method == 0...');
        ...
        //alert('#JCNAlert add_via_ajax = %s', add_via_ajax);
    }
    ...
    if (add_via_ajax) {
        ...
        //alert('#JCNAlert 3 Entrando a colocar productos...');        
        $.ajax({
            method: 'GET',
...
function pos_each_row(row_obj) {
    ...
    //alert('#JCNAlert 2 Entrando a colocar productos...');
...
function pos_total_row() {
    ...
    var price_cost_total=get_cost_subtotal();
    let num = price_cost_total;
   ...
   // console.log(price_cost_total.length);
  ...
  //  $('span.total_cost').html('SS');
       $('span.total_cost').html(y);
...
function get_cost_subtotal() {
    var price_total = 0;

    $('table#pos_table tbody tr').each(function() {
        price_total = price_total + __read_number($(this).find('input.pos_line_purchase_total'));
    });
...
function reset_pos_form(){
    ...
    //#JCN To capture the current category before reset the form. 
    //console.log('Category before clear the form: ' +  $("#product_category option:selected").val() + ' Nombre: ' + $("#product_category option:selected").text()) ;
    var cate_current = $("select#product_category option:selected").val();
 
	if(pos_form_obj[0]){
		pos_form_obj[0].reset();
	}
    //console.log('Category after clear the form: ' +  $("select#product_category option:selected").val() + ' Nombre: ' + $("#product_category option:selected").text()) ;
	if(sell_form[0]){
		sell_form[0].reset();
	}
	set_default_customer();
    ...
    //#JCN to put/preserve the current category 
    $('select#product_category').val(cate_current); //To put in the select trigger the value of current catergory capture above
    $('select#product_category').trigger('change'); //Trigger the event
}
...
$('table#pos_table tbody').on('change', 'input.pos_line_total', function() {
    ...
    var quantity = subtotal / unit_price_inc_tax;
      var total_cost = quantity * unit_cost_price;
...
//#JCN Begin
//Add to show page by page using previos or next page for the productos
...
$(document).on('hidden.bs.modal', '.view_modal', function(){
    ...
    function calc_cloth(product_id,row_id,modifier_id){
      console.log(row_id);

      

      var total=0;
       $('.modal_'+product_id+'_'+row_id).find('.modi_'+product_id+'_'+row_id).each( function(){
         if($(this).val() == 0)
         {
          //  $('.modi_check_'+product_id+'_'+row_id+'_'+modifier_id).prop('checked', false);
         }
         else
         {
          //  $('.modi_check_'+product_id+'_'+row_id+'_'+modifier_id).prop('checked', true); 
         }
            total=parseFloat(total)+ parseFloat($(this).val());
        });
       $('#total_material_'+product_id+'_'+row_id).val(total);
       $('#total_material_'+product_id+'_'+row_id+'_html').html(total);
      
    }

    function enabledisablemodifier(product_id,row_id,modifier_id) {
       
       var inv=$('.modi_'+product_id+'_'+row_id+'_'+modifier_id);
       var dval=inv.attr('data-default');
       console.log(dval);

        var check = $('.modi_check_'+product_id+'_'+row_id+'_'+modifier_id);
        ...
        calc_cloth(product_id,row_id,modifier_id);
    };