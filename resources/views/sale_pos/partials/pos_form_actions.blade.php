<div class="row col-md-12">
	<div class="pos-form-actions {{ $theme_pos_class }}">
		{{-- To organize footer actions buttons --}}
		<div class="col-md-10"> {{-- buttons --}}
			@if(!$is_mobile)
				<div class="bg-navy pos-total text-white  col-md-3">
			...
			<button type="button" 
				class="btn bg-woocommerce text-white btn-default btn-flat no-print @if(!empty($pos_settings['disable_suspend'])) @endif pos-express-finalize @if(!array_key_exists('card', $payment_types)) hide @endif @if($is_mobile) col-xs-6 @endif" 
			...
			@if(empty($edit))
				<button type="button" class="btn btn-default bg-maroon btn-flat @if($is_mobile) col-xs-6 @else btn-md @endif" id="pos-cancel"> <i class="fas fa-window-close"></i> @lang('sale.cancel')</button>
			@else
				<button type="button" class="btn btn-danger btn-flat hide @if($is_mobile) col-xs-6 @else btn-md @endif" id="pos-delete"> <i class="fas fa-trash-alt"></i> @lang('messages.delete')</button>
			@endif
		</div>
		<div class="col-md-2">  {{-- recent_transations --}}
